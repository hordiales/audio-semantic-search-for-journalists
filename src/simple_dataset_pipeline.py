...
def run_pipeline(
    ...
    source_files = [path for path in sorted(input_path.iterdir()) if path.suffix.lower() in SUPPORTED_FORMATS]
    if not source_files:
    ...
    compatible = (
        previous_dataset_path.exists()
        and previous_state.get("pipeline_signature") == signature
    )
    stored_sources = previous_state.get("sources", {})
    previous_sources = stored_sources if compatible else {}
    changed_names = {
        name for name, fingerprint in source_fingerprints.items()
    ...
    if not compatible:
        ...
        logger.info("Incremental cache invalidated by missing state or changed processing configuration")
    ...
    logger.info(
        ...
        len(source_fingerprints) - len(changed_names), len(changed_names), len(removed_names),
    )
    ...
    for name in sorted(changed_names):
        changed_wavs.append(Path(convert_audio(str(source_paths[name]), str(converted_dir), force=True)))
    raw_segments = transcribe_files(changed_wavs, model_name=whisper_model, language=language)
    new_segments = ChunkingProcessor(ChunkingConfig(
        strategy=chunk_strategy,
        duration_sec=chunk_duration_sec,
        overlap_sec=chunk_overlap_sec,
        max_text_chars=max_chunk_text_chars,
    ...
    if new_df.empty:
        new_df = pd.DataFrame(columns=["segment_id", "text", "start_time", "end_time", "language", "confidence", "original_file_name"])
    ...
    if len(new_df) and embedding_config.active & {"clap", "gemini", "yamnet"}:
        ...
        new_df["audio_window_count"] = [len(audio_windows[int(segment_id)]) for segment_id in new_df["segment_id"]]
    ...
    df = pd.concat([unchanged_df, new_df], ignore_index=True, sort=False).sort_values("segment_id").reset_index(drop=True)
    if df.empty:
    ...
    for embedding_dir in embeddings_dir.iterdir():
        ...
        for artifact in embedding_dir.glob("segment_*.npy"):
            ...
            if embedding_dir.name not in embedding_config.active or segment_id not in valid_segment_ids:
                artifact.unlink()
    for artifact in audio_segments_dir.glob("segment_*_window_*.wav"):
    ...
    manifest = {
        ...
        "embeddings": {name: {"model": getattr(embedding_config, f"{name}_model"), "dimension": int(values.shape[1]), "index": index_files[name]} for name, values in all_embeddings.items()},
        "classifiers": (
            {
                "yamnet": {
