...
def verify_dataset(dataset_dir: str):
    ...
    try:
        ...
        if missing_cols:
            ...
            return False
        if os.environ.get('MCP_MODE') != '1':
            print("  ✅ Todas las columnas requeridas presentes", file=sys.stderr)
        ...
        if 'text_embedding' in df.columns and 'audio_embedding' in df.columns:
            ...
            if os.environ.get('MCP_MODE') != '1':
                print("  ❌ Columnas de embeddings no encontradas", file=sys.stderr)
            return False
    ...
    try:
        with open(final_dir / "dataset_manifest.json", encoding='utf-8') as f:
