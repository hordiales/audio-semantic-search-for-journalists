...
from semantic_audio_mapping import (
    suggest_audio_queries,
    validate_audio_query,
)
...
class AudioDatasetClient:
    ...
    def _load_dataset(self):
        ...
        self.log("🧠 Inicializando embedders...")
        ...
        if indices_dir.exists() and len(self.df) > 0:
            self.log("🔍 Cargando índices vectoriales...")
            ...
            self.log("✅ Índices cargados")
        elif len(self.df) == 0:
            self.log("⚠️  Dataset vacío - índices no necesarios")
            self.index_manager = None
        else:
            self.log("⚠️  No se encontraron índices vectoriales")
            self.index_manager = None
        ...
        self.log("🎵 Sistemas de búsqueda de audio inicializados")
    ...
    def print_results(self, results: list[dict], show_details: bool = True):
        ...
        for result in results:
            ...
            if show_details:
                ...
                if result.get('matched_audio_classes'):
